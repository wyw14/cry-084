...
func (s *Service) Submit(ctx context.Context, cmd SubmitCommand) (inspectiondomain.Result, error) {
	...
	err := s.repo.WithinTx(ctx, func(tx application.Repository) error {
		...
		scan := newSubmissionScan(task, cmd)
...
func newSubmissionScan(task inspectiondomain.Task, cmd SubmitCommand) inspectiondomain.Scan {
	return inspectiondomain.Scan{TaskID: task.ID, AssetID: cmd.AssetID, InspectorID: cmd.Actor.ID, QRSecret: cmd.QRSecret, ScannedAt: cmd.ScannedAt.UTC()}
}
