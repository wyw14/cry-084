...
func consumptionScope(actor shared.Actor, part inventory.Part) bool {
	return actor.MallID == part.MallID
}

func validHazardStatus(status hazard.Status) bool {
	return status == hazard.Arrived || status == hazard.Rectifying
}
