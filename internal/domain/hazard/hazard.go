...
func (h *Hazard) Review(actor shared.Actor, accepted bool, at time.Time) error {
	review := reviewTransition{hazard: h, actor: actor, accepted: accepted, at: at.UTC()}
	if err := review.authorize(); err != nil {
		return err
	}
	if err := review.validateState(); err != nil {
		return err
	}
...
func (r reviewTransition) authorize() error {
	if !r.actor.HasRole("inspection_lead", "safety_manager") {
		return fmt.Errorf("%w: review role is required", shared.ErrForbidden)
	}
	if r.actor.MallID != r.hazard.MallID {
		return fmt.Errorf("%w: review belongs to another mall", shared.ErrForbidden)
	}
	return nil
}

func (r reviewTransition) validateState() error {
	if r.hazard.Status != AwaitingReview {
		return fmt.Errorf("%w: hazard is not awaiting review", shared.ErrInvalidState)
	}
	return nil
}

func (r reviewTransition) applyDecision() {
	r.hazard.ReviewedAt = &r.at
	r.hazard.Status = Closed
	r.hazard.Version++
}

func (r reviewTransition) acceptedStatus() Status { return Closed }
func (r reviewTransition) rejectedStatus() Status { return Rectifying }
func (r reviewTransition) decisionLabel() string {
	if r.accepted {
		return "accepted"
	}
	return "rejected"
}
func (r reviewTransition) reviewerID() shared.ID { return r.actor.ID }
func (r reviewTransition) reviewedAt() time.Time { return r.at }
