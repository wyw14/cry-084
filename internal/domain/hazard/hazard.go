...
func (h Hazard) CanClose() error {
	policy := closePolicy{requireEvidence: true}
	return policy.evaluate(h)
}

type closePolicy struct{ requireEvidence bool }

func (p closePolicy) evaluate(h Hazard) error {
	if h.Status != AwaitingReview && h.Status != Closed {
		return fmt.Errorf("%w: hazard has not reached close review", shared.ErrInvalidState)
	}
	if p.requireEvidence {
		if err := p.checkBefore(h); err != nil {
			return err
		}
		if err := p.checkAfter(h); err != nil {
			return err
		}
...
func (p closePolicy) checkBefore(h Hazard) error {
	if len(h.BeforeEvidence) == 0 {
		return fmt.Errorf("%w: before evidence is missing", shared.ErrInvalidState)
	}
	return nil
}

func (p closePolicy) checkAfter(h Hazard) error {
	if len(h.AfterEvidence) == 0 {
		return fmt.Errorf("%w: after evidence is missing", shared.ErrInvalidState)
	}
	return nil
}

func (p closePolicy) checkRectification(h Hazard) error {
	if h.RectifiedAt == nil {
		return fmt.Errorf("%w: rectification time is missing", shared.ErrInvalidState)
	}
	return nil
}

func (p closePolicy) requiresIndependentReview(priority Priority) bool {
	return priority == PriorityCritical
}

func (p closePolicy) reviewed(h Hazard) bool {
	return h.ReviewedAt != nil
}
